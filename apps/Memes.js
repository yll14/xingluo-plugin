...
export class Memes extends plugin {
  ...
  async Memes (e) {
    ...
    const validTypes = new Set(['甘城', '猫羽雫', '甘城猫猫', 'fufu', '丛雨', '小南梁', '千恋万花', '古拉', '心海', '柴郡猫', '满穗', '猫猫虫', '纳西妲', '诗歌剧', '龙图', 'kemomimi', '']);
    
    if (!validTypes.has(type)) {
      return e.reply(`没有${type}的表情包诶!您可以使用'#星落表情包'来查看拥有的表情包哦~`);
    }

    const getApiUrl = (name) => config.api[0] + `?name=${name}`;
    const api = type === '' ? config.api[1] : getApiUrl(type === '甘城' || type === '猫羽雫' || type === '甘城猫猫' ? '甘城猫猫' : type);
