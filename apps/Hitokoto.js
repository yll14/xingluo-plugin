...
export class Hitokoto extends plugin {
  ...
  async Hitokoto(e) {
    ...
    const templateToUse =
      config.templateswitch && config.customize
        ? config.customize
        : config.default;
      ...
      if (json.hitokoto) {
        e.reply(
          `${templateToUse
            ...
            .replace(
              "${time}",
              new Date(parseInt(json.created_at) * 1000).toLocaleString(),
            )
  ...
  async Setting(e) {
    ...
    const match = e.msg.match(
      /(开启|关闭|类型|最小长度|最大长度|句子类型|API地址|自定义模板)\s*(.*)/,
    );
    if (!match) {
  ...
  async ViewSentenceType(e) {
    e.reply(
      `句子类型: a:动画 b:漫画 c:游戏 d:文学 e:原创 f:来自网络 g:其他 h:影视 i:诗词 j:网易云 k:哲学 l:抖机灵\n选择多种使用英文逗号连接例如'a,b,c'`,
    );
  }
  async ViewConfig(e) {
    const { config } = GetConfig(`config`, `Hitokoto`);
