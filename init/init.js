...
export default new class Init {
    ...
    async globalVersion() {
        let PluginVersion = JSON.parse(fs.readFileSync(`./plugins/${PluginName_en}/package.json`, `utf-8`))
        PluginVersion = PluginVersion.version
        global.PluginVersion = PluginVersion
    }
    async globalAuthor() {
        let PluginAuthor = JSON.parse(fs.readFileSync(`./plugins/${PluginName_en}/package.json`, `utf-8`))
        PluginAuthor = PluginAuthor.author
        global.globalAuthor = PluginAuthor
    }
}
