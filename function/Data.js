...
import { PluginName_en } from "./function.js";
const getRoot = (root = "") => {
  if (root === "root" || root === "yunzai") {
    root = `${_path}/`;
  } else if (!root) {
    root = `${_path}/plugins/${PluginName_en}/`;
  }
  return root;
};
...
let Data = {
  ...
  async importCfg(key) {
    let sysCfg = await Data.importModule(`defSet/system/${key}_system.js`);
    let diyCfg = await Data.importModule(`config/${key}.js`);
    if (diyCfg.isSys) {
      console.error(`miao-plugin: defSet/system/${key}.js无效，已忽略`);
      console.error(
        `如需配置请复制defSet/system/${key}_default.js为defSet/system/${key}.js，请勿复制defSet/system下的系统文件`,
      );
      diyCfg = {};
    }
    return {
