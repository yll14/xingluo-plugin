...
const DATA = await fetch('https://web.yll14.cn/?type=json').then(response => response.json())
...
async function ReadMessage(e) {
    if (DATA.code === 200) {
        ...
        if (DATA.Other.push === 'true') {
            console.log(`[LL]公告推送状态为` + DATA.Other.push + `开始推送公告信息:` + DATA.message);
 ...
 * @returns 
 */
async function GetConfig(file, name) {
    const _path = process.cwd().replace(/\\/g, '/')
...
function ReturnInformation(Data, e) {
    ...
    let SERVERCONDITION = Data.hasOwnProperty('server_condition') ? Data.server_condition : '未知';
